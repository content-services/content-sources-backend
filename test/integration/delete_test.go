...
func (s *DeleteTest) TestDeleteSnapshotCleansUntrackedBlockingDistribution() {
	...
	s.staleOutSnapshotDistributionHref(olderSnap)
...
func (s *DeleteTest) staleOutSnapshotDistributionHref(snap models.Snapshot) {
	t := s.T()

	result := db.DB.WithContext(s.ctx).Model(&models.Snapshot{}).
		Where("uuid = ?", snap.UUID).
		Update("distribution_href", "bogus-untracked-distribution-href")
	...
	assert.NotEqual(t, "bogus-untracked-distribution-href", *dist.PulpHref)
	require.True(t, dist.Publication.IsSet())
