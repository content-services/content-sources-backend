...
type LightwellTokenCreateRequest struct {
	...
	AccessLevel string     `json:"access_level"`         // Token scope: "validated" (validated+remediated) or "remediated" (remediated only)
...
type LightwellTokenResponse struct {
	...
	AccessLevel string     `json:"access_level"` // Token scope: "validated" or "remediated"
...
type LightwellTokenValidateRequest struct {
	...
	Path  string `json:"path,omitempty"` // Request path; required for content downloads so access_level can be enforced
}
