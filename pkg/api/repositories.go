...
// SnapshotPublishState reports the aggregate publish state across all snapshots of a partner repository.
// Each field is independent — a repo can be simultaneously published (one snapshot) and publishing (another).
type SnapshotPublishState struct {
	Publishing   bool `json:"publishing"`   // At least one snapshot with published=true has a pending/running task (publish in progress)
	Unpublishing bool `json:"unpublishing"` // At least one snapshot with published=false has a pending/running task (unpublish in progress)
	Published    bool `json:"published"`    // At least one snapshot is published with a completed publish task
	Stopped      bool `json:"stopped"`      // At least one snapshot has a failed/canceled publish task
}
...
type RepositoryResponse struct {
	...
	SnapshotPublishState         *SnapshotPublishState `json:"snapshot_publish_state,omitempty" readonly:"true"`     // Aggregate publish state across all snapshots (partner repos only)
