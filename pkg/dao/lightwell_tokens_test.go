...
func (s *LightwellTokenSuite) TestCreateListValidateRevoke() {
	...
	created, err := dao.Create(context.Background(), orgID, userID, "ci-token", nil)
...
func (s *LightwellTokenSuite) TestExpiredToken() {
	...
	_, err := dao.Create(context.Background(), orgIDTest, "user-exp", "expired", &expired)
	require.Error(s.T(), err)
	daoErr, ok := err.(*ce.DaoError)
	require.True(s.T(), ok)
...
func (s *LightwellTokenSuite) TestCreateRequiresName() {
	dao := lightwellTokenDaoImpl{db: s.tx}
	_, err := dao.Create(context.Background(), orgIDTest, "user", "  ", nil)
	require.Error(s.T(), err)
}
