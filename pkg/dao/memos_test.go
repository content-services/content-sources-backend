...
func (ms *MemoSuite) TestGetLastSuccessfulPulpLogDate_NoMemo() {
	...
	// Verify it's approximately yesterday (within 2 hours tolerance)
	yesterday := time.Now().UTC().Add(-24 * time.Hour)
	diff := date.Sub(yesterday)
	assert.True(ms.T(), diff < 2*time.Hour && diff > -2*time.Hour, "Date should be approximately yesterday")
}
...
func (ms *MemoSuite) TestGetLastSuccessfulPulpLogDate_WithValidMemo() {
	...
	// Create a memo with a valid date
...
func (ms *MemoSuite) TestGetLastSuccessfulPulpLogDate_WithInvalidDateFormat() {
	...
	// Should fall back to yesterday's date
	yesterday := time.Now().UTC().Add(-24 * time.Hour)
	diff := retrievedDate.Sub(yesterday)
	assert.True(ms.T(), diff < 2*time.Hour && diff > -2*time.Hour, "Should fall back to yesterday's date")
}
