...
func (ms *MemoSuite) TestGetLastSuccessfulPulpLogDate_NoMemo() {
	...
	now := time.Now().UTC()
	yesterday := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, now.Location()).Add(-24 * time.Hour)
	diff := date.Sub(yesterday)
	assert.Equal(ms.T(), diff, 0*time.Hour, "Date should be approximately yesterday")
}
