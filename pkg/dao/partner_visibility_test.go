...
func (s *PartnerVisibilitySuite) TestForeignPartnerVisibleExpr() {
	...
	notPublishedRepoConfig := createTestPartnerRepoConfig(t, s.tx, createTestUploadRepository(t, s.tx), ownerOrg, "unpublished foreign partner repo", true)
	uploadRepoConfig := createTestPartnerRepoConfig(t, s.tx, createTestUploadRepository(t, s.tx), ownerOrg, "non-partner repo", false)
	...
	assert.NotContains(t, visibleUUIDs, notPublishedRepoConfig.UUID)
	assert.NotContains(t, visibleUUIDs, uploadRepoConfig.UUID)
}
