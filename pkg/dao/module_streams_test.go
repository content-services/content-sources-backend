...
func (s *RpmSuite) TestSearchModulesForSnapshots() {
	...
	mTangy.On("RpmRepositoryVersionModuleStreamsList", ctx, hrefs, tangy.ModuleStreamListFilters{Search: "Foo", RpmNames: []string{}}, "").Return(expected, 1, nil)
...
func (s *ModuleStreamSuite) TestSearchRepositoryModuleStreams() {
	...
	assert.Equal(t, alpha2.Name, resp.Data[0].ModuleName)
	assert.Equal(t, 2, len(resp.Data[0].Streams))
	assert.Equal(t, alpha2.Version, resp.Data[0].Streams[0].Version)
	assert.Equal(t, alphaNew.Version, resp.Data[0].Streams[1].Version)

	// only 1 stream for beta module
	assert.Equal(t, 1, len(resp.Data[1].Streams))
	assert.Equal(t, beta1.Name, resp.Data[1].ModuleName)
	assert.Equal(t, beta1.Version, resp.Data[1].Streams[0].Version)
	...
	assert.NoError(t, err)
	assert.Equal(t, beta1.Name, resp.Data[0].ModuleName) // beta comes first
	assert.Equal(t, alpha2.Name, resp.Data[1].ModuleName)
