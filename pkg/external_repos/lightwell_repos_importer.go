...
func NewLightwellRepoImporter(daoReg *dao.DaoRegistry, pulpClient pulp_client.PulpClient) LightwellRepoImporter {
...
func (lri *LightwellRepoImporter) LoadAndSave(ctx context.Context) error {
	...
	for _, entry := range entries {
		ecosystem, securityLevel, err := parseLightwellBasePath(entry.BasePath)
		if err != nil {
		...
		publishedDistURL := ""
		dist, err := lri.pulpClient.FindGenericDistributionByBasePath(ctx, entry.BasePath)
		if err != nil {
		...
		_, err = lri.daoReg.RepositoryConfig.InternalOnly_RefreshLightwellRepo(
			ctx, entry.Name, ecosystem, securityLevel, entry.Type, publishedDistURL,
		)
		if err != nil {
...
func parseLightwellBasePath(basePath string) (ecosystem string, securityLevel string, err error) {
