		...
		if rec.Code == http.StatusOK && rec.Body.Len() != 0 {
			...
			if err := json.Unmarshal(rec.Body.Bytes(), &response); err != nil {
				...
				for _, responseOrgId := range responseOrgIds {
					if responseOrgId != orgId && responseOrgId != config.RedHatOrg && responseOrgId != config.CommunityOrg && responseOrgId != config.LightwellOrg && responseOrgId != config.LightwellDemoOrg {
