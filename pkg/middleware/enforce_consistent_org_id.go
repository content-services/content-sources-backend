	...
	return func(c echo.Context) error {
		...
		if rec.Code == http.StatusOK && rec.Body.Len() > 0 {
			...
			if err := json.Unmarshal(rec.Body.Bytes(), &response); err == nil {
				...
				if len(responseOrgIds) > 0 {
					// Check each org_id matches the user's org_id, "-1" for RHEL, or "-2" for community repos.
					for _, responseOrgId := range responseOrgIds {
						if responseOrgId != orgId && responseOrgId != "-1" && responseOrgId != "-2" {
							c.Logger().Errorf("Org ID mismatch: user org_id=%s, response org_id=%s, account_id=%s", orgId, responseOrgId, accountId)
							// Restore original response before returning error
							c.SetResponse(originalResponse)
