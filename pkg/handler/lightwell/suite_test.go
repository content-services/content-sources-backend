...
func (s *LightwellSuite) SetupTest() {
	...
	// Set up default mocks for route registration tests
	// These allow handlers to be called without panicking, but individual tests can override
	s.fsClient.On("GetEntitledFeatures", mock.Anything, mock.Anything).
		Return([]string{}, nil).Maybe()
	s.reg.RepositoryConfig.On("List", mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		Return(api.RepositoryCollectionResponse{}, int64(0), nil).Maybe()
}
