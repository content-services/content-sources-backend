...
func (suite *UserPreferencesSuite) TestMissingUserID() {
	id := test_handler.MockIdentity
	id.Identity.User = &identity.User{Username: "user"}

	path := fmt.Sprintf("%s/user_preferences/", api.FullRootPath())
	req := httptest.NewRequest(http.MethodGet, path, nil)
	req.Header.Set(api.IdentityHeader, test_handler.EncodedCustomIdentity(suite.T(), id))
