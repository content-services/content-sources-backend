...
func (rh *RepositoryHandler) addUploads(c echo.Context) error {
	...
	activeTaskIDs, err := rh.DaoRegistry.TaskInfo.FetchActiveTasks(c.Request().Context(), orgID, response.RepositoryUUID, config.UpdateLatestSnapshotTask, config.BulkRemoveRpmsTask)
	if err != nil {
		return ce.NewErrorResponse(ce.HttpCodeForDaoError(err), "Error checking if bulk-remove-rpms or update-latest-snapshot is in progress", err.Error())
	}
	if len(activeTaskIDs) > 0 {
		return ce.NewErrorResponse(http.StatusConflict, "Error add uploads", "This repository content is currently being changed. Try again later.")
	}
...
func (rh *RepositoryHandler) bulkRemoveRpms(c echo.Context) error {
	...
	activeTaskIDs, err := rh.DaoRegistry.TaskInfo.FetchActiveTasks(c.Request().Context(), orgID, repository.RepositoryUUID, config.UpdateLatestSnapshotTask, config.AddUploadsTask, config.BulkRemoveRpmsTask)
	if err != nil {
		return ce.NewErrorResponse(ce.HttpCodeForDaoError(err), "Error checking if add-uploads, bulk-remove-rpms or update-latest-snapshot is in progress", err.Error())
	}
	if len(activeTaskIDs) > 0 {
		return ce.NewErrorResponse(http.StatusConflict, "Error bulk remove rpms", "This repository content is currently being changed. Try again later.")
	}
