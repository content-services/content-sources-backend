...
// updateLatestDistribution points the repository's public /latest Pulp distribution at
// the effective latest snapshot's publication. It creates or deletes the distribution
// when the repository gains its first or loses its last published snapshot.
func (t *UpdateSnapshotPublished) updateLatestDistribution(repoConfigUUID string) error {
	...
	latestSnap, err := t.daoReg.Snapshot.FetchLatestSnapshotForDistribution(t.ctx, repoConfigUUID)
	if err != nil {
		...
		taskHref, deleteErr := t.pulpClient.DeleteRpmDistribution(t.ctx, dist.GetPulpHref())
		if deleteErr != nil {
			return fmt.Errorf("error deleting latest distribution for repo config %s: %w", repoConfigUUID, deleteErr)
		}
		if taskHref != nil {
			if _, pollErr := t.pulpClient.PollTask(t.ctx, *taskHref); pollErr != nil {
				return fmt.Errorf("error polling latest distribution deletion task: %w", pollErr)
			}
		}
	...
	if dist == nil {
		taskHref, createErr := t.pulpClient.CreateRpmDistribution(
			t.ctx,
		...
		if createErr != nil {
			return fmt.Errorf("error creating latest distribution for repo config %s: %w", repoConfigUUID, createErr)
		}
		if taskHref == nil {
			return fmt.Errorf("error creating latest distribution for repo config %s: Pulp task href is missing", repoConfigUUID)
		}
		if _, pollErr := t.pulpClient.PollTask(t.ctx, *taskHref); pollErr != nil {
			return fmt.Errorf("error polling latest distribution creation task: %w", pollErr)
		}
		return nil
	}
