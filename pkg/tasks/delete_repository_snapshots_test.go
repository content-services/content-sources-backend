...
func (s *DeleteRepositorySnapshotsSuite) TestDeleteSnapshotNotFound() {
	snapshotId := "abacadaba"
	ctx := context.Background()
	repoUuid := uuid.New()
	repo := dao.Repository{UUID: repoUuid.String(), URL: "http://random.example.com/thing"}
	...
	s.mockDaoRegistry.Snapshot.On("FetchForRepoConfigUUID", ctx, repoConfig.UUID, true).Return([]models.Snapshot{expectedSnap}, nil).Once()
	// Simulate snapshot not found - should not error
	notFoundErr := &ce.DaoError{NotFound: true, Message: "Snapshot not found"}
	s.mockDaoRegistry.Snapshot.On("Delete", ctx, expectedSnap.UUID).Return(notFoundErr).Once()
	s.mockDaoRegistry.RepositoryConfig.On("Delete", ctx, repoConfig.OrgID, repoConfig.UUID).Return(nil).Once()
	s.mockDaoRegistry.Template.On("DeleteTemplateSnapshot", ctx, expectedSnap.UUID).Return(nil).Once()
...
func (s *DeleteRepositorySnapshotsSuite) TestDeleteRepoConfigNotFound() {
	ctx := context.Background()
	repoUuid := uuid.New()
	repo := dao.Repository{UUID: repoUuid.String(), URL: "http://random.example.com/thing"}
	repoConfig := api.RepositoryResponse{OrgID: "OrgId", UUID: uuid.NewString(), URL: repo.URL}
	...
	s.mockDaoRegistry.Snapshot.On("FetchForRepoConfigUUID", ctx, repoConfig.UUID, true).Return([]models.Snapshot{}, nil).Once()
	// Simulate repo config not found - should not error
	notFoundErr := &ce.DaoError{NotFound: true, Message: "Repository not found"}
	s.mockDaoRegistry.RepositoryConfig.On("Delete", ctx, repoConfig.OrgID, repoConfig.UUID).Return(notFoundErr).Once()
...
func (s *DeleteRepositorySnapshotsSuite) TestDeleteTemplateSnapshotNotFound() {
	snapshotId := "abacadaba"
	ctx := context.Background()
	repoUuid := uuid.New()
	repo := dao.Repository{UUID: repoUuid.String(), URL: "http://random.example.com/thing"}
	...
	s.mockDaoRegistry.Snapshot.On("Delete", ctx, expectedSnap.UUID).Return(nil).Once()
	s.mockDaoRegistry.RepositoryConfig.On("Delete", ctx, repoConfig.OrgID, repoConfig.UUID).Return(nil).Once()
	// Simulate template snapshot not found - should not error
	notFoundErr := &ce.DaoError{NotFound: true, Message: "Template snapshot not found"}
	s.mockDaoRegistry.Template.On("DeleteTemplateSnapshot", ctx, expectedSnap.UUID).Return(notFoundErr).Once()
