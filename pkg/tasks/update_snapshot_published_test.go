...
func (s *UpdateSnapshotPublishedSuite) TestPublishRemovesContentGuard() {
	...
	// First published snapshot: create the public latest distribution.
	latestPath := repoConfigUUID + "/latest"
	latestTaskHref := uuid.NewString()
	s.mockPulpClient.On("FindDistributionByPath", ctx, latestPath).Return((*zest.RpmRpmDistributionResponse)(nil), nil)
	s.mockDaoRegistry.Snapshot.On("FetchLatestSnapshotForDistribution", ctx, repoConfigUUID).Return(snap, nil)
	s.mockPulpClient.On("CreateRpmDistribution", ctx, pubHref, repoConfigUUID, latestPath, (*string)(nil)).Return(&latestTaskHref, nil)
	s.mockPulpClient.On("PollTask", ctx, latestTaskHref).Return(&zest.TaskResponse{}, nil)
...
func (s *UpdateSnapshotPublishedSuite) TestUnpublishRestoresOrgGuard() {
	...
	// No published snapshots and no latest distribution: nothing to delete.
	latestPath := repoConfigUUID + "/latest"
	s.mockPulpClient.On("FindDistributionByPath", ctx, latestPath).Return((*zest.RpmRpmDistributionResponse)(nil), nil)
	s.mockDaoRegistry.Snapshot.On("FetchLatestSnapshotForDistribution", ctx, repoConfigUUID).Return(models.Snapshot{}, gorm.ErrRecordNotFound)
...
func (s *UpdateSnapshotPublishedSuite) TestUpdateLatestDistributionRetargetsToLatestPublishedSnapshot() {
	ctx := context.Background()
	repoConfigUUID := uuid.NewString()
	latestPath := repoConfigUUID + "/latest"
	...
	s.mockDaoRegistry.Snapshot.On("FetchLatestSnapshotForDistribution", ctx, repoConfigUUID).Return(latestSnap, nil)
...
func (s *UpdateSnapshotPublishedSuite) TestUpdateLatestDistributionDeletesWhenNoPublishedSnapshotsRemain() {
	ctx := context.Background()
	repoConfigUUID := uuid.NewString()
	latestPath := repoConfigUUID + "/latest"
	latestDistHref := "/pulp/distribution/" + uuid.NewString()
	...
	s.mockDaoRegistry.Snapshot.On("FetchLatestSnapshotForDistribution", ctx, repoConfigUUID).Return(models.Snapshot{}, gorm.ErrRecordNotFound)
	s.mockPulpClient.On("DeleteRpmDistribution", ctx, latestDistHref).Return(&taskHref, nil)
	s.mockPulpClient.On("PollTask", ctx, taskHref).Return(&zest.TaskResponse{}, nil)
...
func (s *UpdateSnapshotPublishedSuite) TestUpdateLatestDistributionCreatesWhenMissing() {
	ctx := context.Background()
	repoConfigUUID := uuid.NewString()
	latestPath := repoConfigUUID + "/latest"
	publicationHref := "/pulp/publication/" + uuid.NewString()
	...
	s.mockPulpClient.On("FindDistributionByPath", ctx, latestPath).Return((*zest.RpmRpmDistributionResponse)(nil), nil)
	s.mockDaoRegistry.Snapshot.On("FetchLatestSnapshotForDistribution", ctx, repoConfigUUID).Return(latestSnap, nil)
	s.mockPulpClient.On("CreateRpmDistribution", ctx, publicationHref, repoConfigUUID, latestPath, (*string)(nil)).Return(&taskHref, nil)
	s.mockPulpClient.On("PollTask", ctx, taskHref).Return(&zest.TaskResponse{}, nil)
...
func (s *UpdateSnapshotPublishedSuite) TestUpdateLatestDistributionFetchLatestSnapshotError() {
	...
	s.mockDaoRegistry.Snapshot.On("FetchLatestSnapshotForDistribution", ctx, repoConfigUUID).Return(models.Snapshot{}, fmt.Errorf("database error"))
