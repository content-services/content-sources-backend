...
func logRequestId(msg string, lg zerolog.Logger, ctx context.Context) {
	rId, ok := ctx.Value(config.HeaderRequestId).(string)
	if ok {
...
func (sr *SnapshotRepository) findOrCreateRemote(repoConfig api.RepositoryResponse) (string, error) {
	...
	if remoteResp == nil {
		remoteResp, err = sr.pulpClient.CreateRpmRemote(context.TODO(), repoConfig.UUID, repoConfig.URL, clientCertPair, clientCertPair, caCert)
		if err != nil {
			return "", err
		}
