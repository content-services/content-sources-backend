export const randomName = () => (Math.random() + 1).toString(36).substring(2, 6);

export const SmallRedHatRepoURL =
  'https://cdn.redhat.com/content/dist/rhel9/9/aarch64/codeready-builder/os/';

/**
 * A small, valid GPG public key for testing purposes.
 * Source: https://jlsherrill.fedorapeople.org/fake-repos/needed-errata/RPM-GPG-KEY-dummy-packages-generator
 * Can be used to test packages from https://jlsherrill.fedorapeople.org/fake-repos/
 */
export const TestGpgKey = `-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1.4.11 (GNU/Linux)


=RrIG
-----END PGP PUBLIC KEY BLOCK-----`;
