...
Use any RPM you need to upload. The examples use `example.rpm`.
...
Split the file into chunks. The examples use 6 MiB chunks (`6291456` bytes), which is the recommended size.

```bash
CHUNK_SIZE=6291456
split -b "$CHUNK_SIZE" "$RPM" chunk_
ls -l chunk_*
```
...
curl -s -X GET "$API/tasks/$TASK_UUID" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -s -X POST "$API/repositories/uploads/" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"size\": $FILE_SIZE,
...
Run this loop from the directory that contains the `chunk_*` files. It advances `offset` so each chunk is placed after the previous one. Do not reuse `START=0` for every chunk.
...
for chunk in chunk_*; do
  ...
  curl -sS -X POST "$API/repositories/uploads/$UPLOAD_UUID/upload_chunk/" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Range: bytes ${offset}-${end}/*" \
    --form "file=@${chunk}" \
    --form "sha256=${chunk_sha256}"
  echo
...
```bash
curl -s -X POST "$API/repositories/$REPO_UUID/add_uploads/" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{
    \"uploads\": [
...
You can include several uploads in one request. Use `uuid` (the public upload id). Do not send Pulp `href` values; those are only used by internal services.
...
curl -s -X GET "$API/tasks/$ADD_TASK_UUID" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -s -X GET "$API/repositories/$REPO_UUID/rpms" \
  -H "Authorization: Bearer $TOKEN"
```

## Notes

- Create-upload and `add_uploads` `sha256` is the checksum of the **complete** RPM. Chunk `sha256` is the checksum of **that chunk**.
...
- Use the public Content Sources paths only:
